...
ax.contour3D(X, Y, Z3, 100, cmap='viridis')### set fn here
ax.set_xlabel('x')
ax.set_ylabel('y')
ax.set_zlabel('z')
ax.set_title('gradient descent for f3(x), step size = 0.3');### set fn here
...
def gradient_descent(s,t,fn):
	for i in range(50):
		...
		s=s-(0.3)*g1 #0.6/np.sqrt(i+1)
		t=t-(0.3)*g2 #0.6/np.sqrt(i+1)
		if(fn==2):
			#print('2')
			if(i==49):
				plot_point2(s,t,'blue')
			if(g1 > 0.0000000000000000000000000001 or g2 > 0.0000000000000000000000000001):
				plot_point2(s,t,'red')
			else: 
...
plot_point3(1.0,-1.0,'red')
'''
plot_point(1.0,-1.0,'black')
gradient_descent(1, -1, 2)
plot_point2(1.0,-1.0,'black')
'''
